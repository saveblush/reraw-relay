...
func (s *service) FindByID(c *cctx.Context, ID string) (*models.Event, error) {
	res, err := s.repository.FindByID(c.GetRelayDatabase(), ID)
	if err != nil {
		return nil, err
	}
...
func (s *service) Count(c *cctx.Context, req *Request) (*int64, error) {
	res, err := s.repository.Count(c.GetRelayDatabase(), req)
	if err != nil {
		return nil, err
	}
...
func (s *service) ClearEventsWithBlacklist(c *cctx.Context) error {
	...
	// delete event
	for _, v := range fetch {
		err := s.repository.SoftDelete(c.GetRelayDatabase(), &models.Event{ID: v.ID})
		if err != nil {
